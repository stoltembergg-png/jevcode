...
async function probeModel(host: string, port: number, modelPath: string | undefined): Promise<"match" | "other" | "none"> {
  ...
  if (served && modelPath && normalizePath(served) === normalizePath(modelPath)) return "match"
  return "other"
}
...
export function createSidecar(cfg: SemifResolved): Sidecar {
  ...
  async function spawnServer(port: number): Promise<void> {
    ...
    // assertSemifPaths throws when unset; re-read into locals so the types narrow.
    const serverPath = cfg.serverPath
    const modelPath = cfg.modelPath
    if (!serverPath || !modelPath) throw new Error("semif: model/server path is not configured")
    currentPort = port
    ...
    const next = spawn(
      ...
      { stdio: "ignore", cwd: dirname(serverPath), windowsHide: true },
    )
    child = next
    spawned = true
  ...
  async function dispose(): Promise<void> {
    ...
    const current = child
    child = undefined
    // Only kill processes this module spawned; never kill an adopted external server.
