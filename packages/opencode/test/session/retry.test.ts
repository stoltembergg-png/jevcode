...
describe("session.retry.retryable", () => {
  ...
  test("maps free limits to Go upsell action", () => {
    ...
    expect(SessionRetry.retryable(error, "opencode")).toEqual({
      ...
      action: {
        ...
        message: "Subscribe to NextCode Go for reliable access to the best open-source models for $10/month.",
        label: "subscribe",
  ...
  test("maps Go subscription limits to workspace PAYG upsell", () => {
    ...
    expect(SessionRetry.retryable(error, "opencode-go")).toEqual({
      message:
        "5 hour usage limit reached. It will reset in 5 hours 23 minutes. To continue using this model now, enable usage from your available balance - https://github.com/stoltembergg-png/nextcode",
      action: {
        ...
        label: "open settings",
        link: "https://github.com/stoltembergg-png/nextcode",
      },
    })
  })
